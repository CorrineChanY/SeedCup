...
　　　　　　this.count();
   ...
   */
  async signup(){
    const {username, password, school, college, className, phoneNumber, smsCaptcha, email, confpwd} = this.state
    if(username === '' || school === '' ||
        college === '' || className === '' ||
        password === '' || confpwd === '')
      ...
      message.error('请输入正确的邮箱 (´・ω・`)  ');
    else if(smsCaptcha === '')
      message.error('验证码不能为空 (´・ω・`)  ');
            ...
            <form >
                <div className="captain">
                    ...
                    <div style={{display:"flex"}}>
                        <div style={{flex:"1"}}>
                            ...
                            <input 
                              ...
                              onBlur={() => this.setState({phoneInfo : !checkPhone(this.state.phoneNumber)})}
                            ></input>
                            { this.state.phoneInfo && 
                              <span className="phoneinfo">请输入正确的电话 (´・ω・`)  </span>
                            }
                        </div>
                        <button className="inputafterinfo" id="vcodeget" type="button" onClick={this.getCAPTCHA}>
                          {this.state.loading ? this.state.yztime + '秒后可重新发送' : '获取验证码'}
                        </button>
                    </div>
                    <br />
                    <label>验证码</label><br />
                    ...
                    <input
                      ...
                      onBlur={() => this.setState({emailInfo : !checkEmail(this.state.email)})}
                    ></input>
                    ...
                    <input 
                      ...
                      onBlur={() => this.setState({pwdInfo: this.state.confpwd !== '' && this.state.password !== this.state.confpwd})}
                    ></input>
                    <br /><br />
                    <label>确认密码</label>
                    <br />
                    <input 
                      ...
                      onBlur={() => this.setState({pwdInfo: this.state.password !== this.state.confpwd})}
                    ></input>
                    { this.state.pwdInfo &&
